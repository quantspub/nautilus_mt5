...
class DockerizedMT5Terminal:
    ...
    CONTAINER_NAME: ClassVar[str] = "platform-mt5"
    PORTS: ClassVar[dict[str, int]] = {
        "web": 8000,
        "vnc": 5900,
        "rpyc": DEFAULT_SERVER_PORT,
    }
    ...
    def __init__(self, config: DockerizedMT5TerminalConfig):
        ...
        if self.account_number is None:
            self.log.error(
                "`account_number` not set nor available in env `MT5_ACCOUNT_NUMBER`"
            )
            raise ValueError(
                "`account_number` not set nor available in env `MT5_ACCOUNT_NUMBER`"
            )
    ...
    def is_logged_in(container) -> bool:
        ...
        try:
            ...
            return False
        return any(
            b"Login successful: True" in line for line in logs.split(b"\n")
    ...
    def start(self, wait: int | None = None, restart_policy: Dict = {}) -> None:
        ...
        for image in [self.IMAGE, self.FALLBACK_IMAGE]:
            ...
            raise RuntimeError(
                "Failed to start the MT5 Terminal container with any image."
            )
        ...
        for _ in range(wait or self.timeout):
            ...
            raise RuntimeError(
                f"Terminal `{self.CONTAINER_NAME}-{self.port}` not ready"
            )
    ...
    def stop(self) -> None:
        if self.container:
            try:
                ...
                self.log.error(
                    f"Container `{self.CONTAINER_NAME}-{self.port}` not found."
                )
