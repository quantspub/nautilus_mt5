...
class MetaTrader5InstrumentProvider(InstrumentProvider):
    ...
    async def initialize(self) -> None:
        ...
        if not self._load_ids_on_start and self._load_symbols_on_start:
            ...
            await self.load_ids_async(
                []
            )  # Asynchronously load symbols with an empty load_ids
            self._loading = False
            self._loaded = True
    ...
    async def get_symbol_details(
        ...
        try:
            ...
            self._log.error(
                f"No symbol details found for the given kwargs {symbol}, {e}"
            )
            return []
    ...
    async def load_async(
        ...
        if isinstance(instrument_id, InstrumentId):
            ...
            self._log.error(
                f"Expected InstrumentId or MT5Symbol, received {instrument_id}"
            )
            return
        ...
        for details in copy.deepcopy(symbol_details):
            ...
            try:
                ...
                self._log.error(
                    f"{self.config.strict_symbology=} failed to parse {details=}, {e}"
                )
                continue
